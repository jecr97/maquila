...
import {
  faScissors, faPlus, faPenToSquare, faToggleOn, faToggleOff, faTrash,
...
export default function TiposCorte() {
  ...
  const handleToggle = async (id) => {
    await fetch(`${API_URL}/api/tipos-corte/${id}/status`, { method: 'PATCH' });
    fetch_();
  };

  const handleDelete = async (id) => {
    await fetch(`${API_URL}/api/tipos-corte/${id}`, { method: 'DELETE' });
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <Box sx={{ width: 48, height: 48, borderRadius: '12px', background: 'linear-gradient(135deg, #1565c0, #42a5f5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <FontAwesomeIcon icon={faScissors} style={{ color: '#fff', fontSize: 20 }} />
          </Box>
          <Box>
            <Typography variant="h5" fontWeight={700} color="primary.main">Tipos de Corte</Typography>
      ...
      <Dialog open={!!confirmId} onClose={() => setConfirmId(null)} maxWidth="xs" fullWidth PaperProps={{ sx: { borderRadius: '14px' } }}>
        <DialogTitle sx={{ fontWeight: 700, color: '#c62828' }}>¿Eliminar tipo de corte?</DialogTitle>
